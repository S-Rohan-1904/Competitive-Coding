...
const int INF = 1e9 + 7; // Increased INF to prevent overflow/false minimums

int rec(int i) {
    // Base case: If we've reached the last stone, cost to end is 0
    if(i == n - 1) return 0;
    
    // Cache check
    if(dp[i] != -1) return dp[i];
    
    // 1-step jump is always valid if we aren't at the last stone yet
    int ans = abs(h[i] - h[i+1]) + rec(i+1);
    
    // 2-step jump (only valid if we don't jump past the last stone)
    if(i + 2 < n) {
