...
// dp(i,j) -> represents number of ways from (0,0) to (i,j) with down as the last move such that there are not more than X consecutive right moves
int n, m, x;
int dp[1001][1001];
...
int prefix_sum(int i, int j) {
    ...
    return prefix[i][j] = prefix_sum(i, j - 1) + rec(i, j);
}
...
void solve() {
    ...
    for (int i = 1; i < n; ++i) {
        ...
        for (int j = 0; j < m; ++j) {
            window_sum += dp[i-1][j];
            if (j - x - 1 >= 0) window_sum -= dp[i-1][j - x - 1];
            dp[i][j] = window_sum;
        }
    }
    int ans = 0;
    for (int k = 0; k <= x && m-1-k >= 0; ++k) {
        ans += dp[n-1][m-1-k];
    }
